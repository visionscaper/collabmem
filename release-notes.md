...
**Duplicate-install detection (#47).** If two collabmem installs are active over the same memory — typically a user-level `~/.claude/` install left in place next to a project-level one — the memory sits in context twice, both hooks fire, and one of the two quietly stops getting upgraded. The AI now detects this at session start and tells the user before doing anything else, so the redundant install can be removed instead of lingering for months. How: the session-start hook and the `COLLABMEM-LOAD-CHECK` block both check three signals — whether the `COLLABMEM-LOAD-CHECK` section itself, the methodology marker, or the collabmem hook output appears more than once in context. The section count is the reliable one: the harness may merge two imports of the same memory file into one (so the marker can still count once), but instruction files are never merged. A new "duplicate installs" note in the troubleshooting guide walks through keep-one-remove-the-other. `install.md` now also says plainly: always install at project level, never in a user-level instruction file.
...
**Bug fix: the `.collab-config` import in the instruction-block template was wrong since v1.0.** The template line read `@collab/.collab-config`, but the file lives at the project root, so on code-repo installs that import silently loaded nothing — the AI ran without `collab_dir`, the thresholds, and `project_starred` in context and nobody noticed, because the hook and the AI read the file from disk when needed. Found by a simulated-user install run. The template now reads `@.collab-config` (`@../.collab-config` for an instruction file in `.claude/`). **Existing installs must correct this line** — see the upgrade steps below.

**Bug fix: the hook no longer needs `jq`.** Since v1.1 the session-start hook parsed its input with `jq`, which is not present by default on macOS before Sequoia (15) or on many Linux installs. On such machines the hook failed at every session start with "jq: command not found", and the memory system looked broken — never documented. The hook now extracts what it needs with standard shell tools that every OS ships (`grep`, `sed`, `tr`, `head`). No action needed beyond the hook re-copy in the upgrade steps below. (Rule going forward: shipped components may not depend on anything that is not on the OS by default.)

**Also:** a probe that fails to authenticate or errors before answering is not a load-check result — the AI now finds the cause, helps fix it, and re-runs; without a CLI at all, a fresh session's banner is the fallback (`install.md` Step 8, `upgrade.md` Step 5). Installation now checks for an existing user-level install first, and the default for a project that already has a hook is "keep both as separate files" (never copy the collab-memory logic into another script — that breaks the stamp and the upgrade's hook check). Team installs get one explicit choice — track the memory-system traces (`.collab-config`, instruction file, `.claude/`) in the code repo or git-ignore them — and the commit rule follows from it. And the `permissions.additionalDirectories` recommendation from v1.8.5 is **withdrawn**: tested on Claude Code 2.1.263, it has no effect on import loading either way; the per-project external-includes approval alone is the fix, and the entry only put a machine-specific path into `settings.json`.
...
- **clients/claude-code/hooks/collab-memory-hook.sh:** header version stamp; load-check step d) — duplicate-install check; `jq` dependency removed; `starmem` added to the sentinel list.
...
4. **Refresh the import block:** add the duplicate-install paragraph to the `COLLABMEM-LOAD-CHECK` section (from install.md's template, after the multi-install paragraph), and add the version-stamp line `collabmem instruction block, checked and updated up to: v1.8.6` as the first line after `<!-- collab-memory-system:start -->`. Keep the existing paths and user additions. **Then check the `.collab-config` import line:** if it reads `@collab/.collab-config` (or `@../collab/.collab-config`), it points at a file that does not exist — change it to `@.collab-config` (instruction file at the project root) or `@../.collab-config` (instruction file in `.claude/`), i.e. the same path prefix as the other imports minus the `collab/` segment. Installs whose `.collab-config` sits next to the collab directory in a memory repo (e.g. `@<abs-path>/projects/<name>/.collab-config`) are already correct — leave them.
...
**Changes since v1.8.5 (commit `a907d24`):** documentation only, listed below.
...
- **`additionalDirectories` is not sufficient on its own.** *(Withdrawn in v1.8.6: tested to have no effect at all — do not add the entry.)* v1.8.5 framed `permissions.additionalDirectories` as "the sanctioned, durable route." Field-observed on a real install: with it correctly set, imports still did not resolve until the per-project `hasClaudeMdExternalIncludesApproved` was `true`. All surfaces now say: it states intent and may help across harness updates, but **set both and verify with the probe** — never assume either alone worked. (install.md, upgrade.md, troubleshoot.md.)
...
Also new: a symptom-first **troubleshooting guide** (`clients/claude-code/troubleshoot.md`, copied into `<collab>/docs/` at install), a **`clients/<client>/` source layout** (the hook moved to `clients/claude-code/hooks/`), and a documented recommendation *(withdrawn in v1.8.6 — no effect, do not add)* to declare the shared-knowledge directory via **`permissions.additionalDirectories`** (states intent through a supported mechanism; see the v1.8.5a note — observed not sufficient by itself, set it alongside the external-includes approval and verify with the probe).
