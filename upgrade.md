...
**Find the installed version's commit.** It is the hash in the "Changes since vX (commit …)" line of the `release-notes.md` section *directly after* the installed version vX — every section states its predecessor's commit, patches included. Do not derive the commit in any other way (from the marker-bump commit, the git log, or a guess): that is not reliable and leads to false customisations being reported, or real ones being missed.
