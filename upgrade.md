...
5. **For team/shared-knowledge installs, commit and push the shared-knowledge repo** after the shared-dir files are updated (only that repo), so teammates pick up the new memory-side files. **For a standalone memory project, commit and push everything** — shared part and per-clone part (instruction file, hook) live in the same repository there.
