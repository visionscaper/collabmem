...
**Clone this repository with full history — not `git clone --depth 1`.** Step 3 compares the user's installed files with the originals of their installed version, which needs the older commits; a shallow clone silently breaks that check (details in Step 3). If the clone you are reading this from is shallow, run `git fetch --unshallow` in it before continuing.
...
**Find the installed version's commit.** It is the hash in the "Changes since vX (commit …)" line of the section for the **next newer version** after the installed version vX (`release-notes.md` is ordered newest first, so that section sits *above* vX's) — every section states its predecessor's commit, patches included. Do not derive the commit in any other way (from the marker-bump commit, the git log, or a guess): that is not reliable and leads to false customisations being reported, or real ones being missed.

Diff from that commit to HEAD in this repository. Use the release notes as context to understand the changes.

Check for user customisations: diff the user's installed system files against the originals from the installed version's commit found above. Any differences indicate user customisations that must be preserved or merged during the upgrade.
