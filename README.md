> **v1.8.6 — new: the standalone memory project.** collabmem is no longer only for code repositories: a memory repo can *be* the project — for organisation-level memory, research or business projects, and non-technical users. See "Choosing a Setup" below. Also new: detection of duplicate installs, and per-clone version stamps for distributed setups. See [`release-notes.md`](release-notes.md).
>
